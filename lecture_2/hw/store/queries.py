...
def get_items(offset: int = 0, limit: int = 10, min_price: float = None, max_price: float = None,show_deleted: bool = False):
...
def get_carts(offset: int = 0, limit: int = 10, min_price: float = None, max_price: float = None, min_quantity: int = None, max_quantity: int = None) -> List[Cart]:
...
def patch_item(id: int, patch_item: ItemRequest):
    ...
    return Item(id=id, name = items[id].name, price = items[id].price, deleted = items[id].deleted)
