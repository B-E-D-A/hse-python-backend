...
def get_admin_secret(username: str, password: str) -> dict:
    return {"Authorization": f"Basic {base64.b64encode(f"{username}:{password}".encode("utf-8")).decode("utf-8")}"}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            400,
            "both id and username are provided"
        ),
        (
            {"author_role": UserRole.USER},
            400,
            "neither id nor username are provided"
        ),
    ],
)
@pytest.mark.asyncio
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"Authorization": "Basic SoMeWRONGLet5Er35s"},
            401
        ),
        (
            get_admin_secret("admin", "WrongSecretAdminPassword123"),
            401
        ),
    ],
)      
